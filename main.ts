...
const SENSITIVE_TOOLS = new Set(["run_terminal_cmd"]);
...
async function startCli(config: RuntimeConfig): Promise<void> {
  ...
  agent.loopInterceptor.register(
    new ToolExecutionInterceptor(
      agent.mcp,
      (name, args, options) =>
        handleToolApproval(name, args, config, options?.signal),
    ),
  );

  console.log(`\n🧠 Provider: ${config.provider}`);
  console.log(`🗂️  Workspace: ${context.workingDirectory}`);
  console.log(`🧩 Model: ${config.model}\n`);

  await runAgentInTerminal(agent, config.model);
}
...
async function handleToolApproval(
  ...
  const requiresApproval = shouldPromptForApproval(name, parameters);
  if (!requiresApproval) {
    return true;
  }
  ...
  while (!signal?.aborted) {
    const answer = prompt("Approve tool execution? (y/N): ") ?? "n";
    const normalized = answer.trim().toLowerCase();
    if (normalized === "y" || normalized === "yes") {
      return true;
    }
    if (
      normalized === "n" || normalized === "no" || normalized === ""
    ) {
      return false;
    }
  }
...
function shouldPromptForApproval(
  name: string,
  parameters: Record<string, unknown>,
): boolean {
